...
import com.mrxu.sql.translate.ElasticLexer;
import com.mrxu.sql.translate.SQLOdbcExpr;
import com.mrxu.sql.translate.SQLParensIdentifierExpr;
...
public class ElasticSqlExprParser extends SQLExprParser {
    ...
    public final SQLExpr primaryRest(SQLExpr expr) {
        ...
        if (lexer.token() == Token.DOT && expr instanceof SQLIdentifierExpr) {
            if (StringUtils.isQuoted(((SQLIdentifierExpr) expr).getName(), "`")) {
                lexer.nextToken();
