...
import static com.mrxu.sql.utils.StringUtils.format;
import static com.mrxu.sql.utils.StringUtils.isQuoted;
...
public class SQLFunctions {
    ...
    public Tuple<String, String> log10(SQLExpr field) {
        ...
        return new Tuple<>(name, def(name, format("Math.log10(%s)", getPropertyOrValue(field))));
    }

    public Tuple<String, String> ln(SQLExpr field) {
        String name = nextId("ln");
        return new Tuple<>(name, def(name, format("Math.log(%s)", getPropertyOrValue(field))));
    }
    ...
    private Tuple<String, String> locate(String pattern, SQLExpr source, int start) {
        ...
        return new Tuple<>(name, def(name, format("%s.indexOf(%s,%d)+1", docSource, pattern, start)));
    }
    ...
    private Tuple<String, String> maketime(SQLExpr hr, SQLExpr min, SQLExpr sec) {
        ...
        return new Tuple<>(name, def(name, format(
    ...
    private String getCastToNumericValueScript(String varName, String docValue, String targetType) {
        ...
        return format(script, varName, docValue, targetType);
    }
        ...
        switch (StringUtils.toUpper(castType)) {
            ...
            default:
                throw new UnsupportedOperationException(
                    format("The following type is not supported by cast(): %s", castType)
                );
        }
    }
