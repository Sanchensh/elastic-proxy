    ...
    public static Object generateSpatialParamsObject(String methodName, List<SQLExpr> params) {
        switch (methodName) {
            ...
            case "GEO_BOUNDING_BOX":
                if (params.size() != 5) {
                    throw new RuntimeException("GEO_BOUNDING_BOX should have exactly 5 parameters : "
                            + "(fieldName,topLeftLon,topLeftLan,bottomRightLon,bottomRightLan) ");
                }
                ...
                return new BoundingBoxFilterParams(new Point(topLeftLon, topLeftLat),
                        new Point(bottomRightLon, bottomRightLat));
            case "GEO_DISTANCE":
                if (params.size() != 4) {
                    throw new RuntimeException("GEO_DISTANCE should have exactly 4 parameters : "
                            + "(fieldName,distance,fromLon,fromLat) ");
                }
            ...
            case "GEO_DISTANCE_RANGE":
                if (params.size() != 5) {
                    throw new RuntimeException("GEO_DISTANCE should have exactly 5 parameters : "
                            + "(fieldName,distanceFrom,distanceTo,fromLon,fromLat) ");
                }
            ...
            case "GEO_POLYGON":
                if (params.size() % 2 == 0 || params.size() <= 5) {
                    throw new RuntimeException("GEO_POLYGON should have odd num of parameters and > 5 : "
                            + "(fieldName,lon1,lat1,lon2,lat2,lon3,lat3,...) ");
                }
            ...
            case "GEO_CELL":
                if (params.size() < 4 || params.size() > 5) {
                    throw new RuntimeException("GEO_CELL should have 4 or 5 params "
                            + "(fieldName,lon,lat,precision,neighbors(optional)) ");
                }
